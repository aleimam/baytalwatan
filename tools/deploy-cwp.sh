...
if [ -z "$DOCROOT" ]; then
  ...
  [ -z "$CONF" ] && CONF=$(grep -rilE "$DOMAIN" /usr/local/apache/conf.d/vhosts/ 2>/dev/null | head -1)
  [ -z "$CONF" ] && CONF=$(grep -rilE "$DOMAIN" /usr/local/apache/conf*/ 2>/dev/null | head -1)
  [ -n "$CONF" ] && DOCROOT=$(awk 'tolower($1)=="documentroot"{gsub(/"/,"",$2);print $2;exit}' "$CONF")
fi
if [ -z "$DOCROOT" ] || [ ! -d "$DOCROOT" ]; then
  echo "!! Could not auto-detect the document root for $DOMAIN."
  echo "   Find it:  grep -ri documentroot /usr/local/apache/conf.d/vhosts/ | grep $DOMAIN"
  echo "   Re-run :  DOCROOT=/home/USER/public_html bash <(curl -fsSL <this-url>) $DOMAIN"
  exit 1
fi
